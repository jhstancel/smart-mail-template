...
def main():
    ...
    # safety dedupe
    df["canon"] = df.apply(lambda r: hash_canon(r.get("subject",""), r.get("body","")), axis=1)
    ...
    n_labels = len(train)
    if n_labels < 25:
        print("Warning: very few labeled rows; classifier may be weak.", file=sys.stderr)
    # ensure at least 2 classes
    classes = sorted(train["intent"].unique().tolist())
    if len(classes) < 2:
        print(f"[error] need at least 2 intent classes to train; found: {classes}", file=sys.stderr)
        sys.exit(3)
