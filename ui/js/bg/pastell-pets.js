export const PastellPets = (function(){
  ...
  function restartAnimation(img){ img.style.animation='none'; void img.offsetWidth; img.style.animation=''; }
  function recycle(img){ img.classList.remove('pop'); img.style.top = offscreenY(img) + 'px'; restartAnimation(img); }
  function placeInitial(img, x){
    img.style.left = x + 'px'; img.style.visibility = 'hidden';
    const setY = ()=>{ img.style.top = offscreenY(img) + 'px'; img.style.visibility = 'visible'; restartAnimation(img); };
    if (img.complete && (img.naturalHeight || img.getBoundingClientRect().height)) setY();
    else { img.addEventListener('load', setY, { once:true }); queueMicrotask(()=>{ if (img.style.visibility === 'hidden') setY(); }); }
  }
  function spawnOnce(){
  ...
  function enable(){ if (enabled) return; enabled = true; ensureLayer().style.display = 'block'; if (!flakes.length) spawnOnce(); }
  function disable(){ enabled = false; spawned = false; if (layer) layer.style.display = 'none'; }
  return { enable, disable };
})();
window.PastellPets = PastellPets;
